...
func main() {
    interfaceName := flag.String("i", "", "指定网络接口名称")
    ...
    for _, iface := range interfaces {
        if strings.TrimSpace(iface.Name) == strings.TrimSpace(*interfaceName) {
    ...
    if selectedInterface == nil {
        log.Fatalf("未找到名称为 '%s' 的网络接口", *interfaceName)
    }

    fmt.Printf("选定网络接口: %s - (%s)\n", selectedInterface.Name, getIPv4(*selectedInterface))
